...
function testStudentCommand() {
    ...
    if [[ $STEP == 2 ]]; then
        cd "$EXERCISE_FOLDER"
        if [[ "$CMD" == "mkdir "* ]]; then
            # bash doesn't like it when there's a space in the path when the entire command is in a variable
            MKPATH=${CMD/'mkdir '/''} # get what follows mkdir, assuming it's a path
            if [[ "$MKPATH" == "~"* ]]; then
                MKPATH=${MKPATH/'~'/$HOME} # Tilde expansion only happens when the tilde is unquoted, replace with $HOME (https://stackoverflow.com/a/8409034/1034308)
            fi
            mkdir "$MKPATH"
        fi
        if [[ -d ~/work ]]; then
